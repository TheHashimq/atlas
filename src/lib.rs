...
pub fn start() {
    ...
    spawn_local(async move {
        ...
        // Primary warm point light (follows sphere)
        scene.add_point_light(
            light_t.clone(),
            PointLight { color: [1.0, 0.92, 0.6], intensity: 40.0 },
        );

        // Cool blue fill light — static
        let fill_t = Rc::new(RefCell::new(Transform::identity()));
        fill_t.borrow_mut().translation = glam::Vec3::new(-6.0, 5.0, -2.0);
        scene.add_point_light(
            fill_t,
            PointLight { color: [0.3, 0.5, 1.0], intensity: 12.0 },
        );
